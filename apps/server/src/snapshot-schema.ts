...
const reconnectTokenHashSchema = z
  .string()
  .min(1)
  .regex(/^[A-Za-z0-9+/]+={0,2}$/);
...
const pendingWitchActionSchema = z
  .object({
    saved: z.boolean(),
...
const pendingHunterResolutionSchema = z
  .object({
...
export const lobbyRoomSnapshotSchema = z
  .object({
...
export const gameRuntimeSnapshotSchema = z
  .object({
    version: snapshotVersionSchema,
