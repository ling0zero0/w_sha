...
describe("AES-GCM AI secret box", () => {
  ...
  it("binds ciphertext to its purpose, key and authentication tag", () => {
    ...
    expect(() => box.open("provider:beta:api-key", encrypted)).toThrow("could not decrypt AI credential");
    expect(() => otherBox.open("provider:alpha:api-key", encrypted)).toThrow("could not decrypt AI credential");
    expect(() =>
      box.open("provider:alpha:api-key", {
        ...encrypted,
  ...
  it("rejects invalid master key material and versions", () => {
    expect(() => new AesGcmSecretBox(randomBytes(31))).toThrow("AI master key must contain exactly 32 bytes");
    expect(() => new AesGcmSecretBox(randomBytes(32), 0)).toThrow("AI master key version must be a positive integer");
    expect(() => AesGcmSecretBox.fromBase64("not canonical base64")).toThrow("AI master key must be canonical base64");
  });
});
