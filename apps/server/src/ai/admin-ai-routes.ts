...
export function registerAiAdminRoutes(app: FastifyInstance, runtime: GameRuntime, services: AiAdminServices): void {
  void app.register(
    async (admin) => {
      ...
      admin.post("/providers", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
      ...
      admin.patch("/providers/:id", async (request, reply) =>
      ...
      admin.delete("/providers/:id", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
      ...
      admin.post("/providers/:id/test", async (request, reply) =>
        execute(request, reply, async () => {
          const providerId = parseProviderId(request.params);
          const model = services.store.listModelProfiles().find((candidate) => candidate.providerId === providerId);
      ...
      admin.post("/models", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
      ...
      admin.patch("/models/:id", async (request, reply) =>
        execute(request, reply, () => {
      ...
      admin.delete("/models/:id", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
      ...
      admin.post("/models/:id/test", async (request, reply) =>
        execute(request, reply, () => {
      ...
      admin.post("/bot-profiles", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
      ...
      admin.patch("/bot-profiles/:id", async (request, reply) =>
        execute(request, reply, () => {
      ...
      admin.delete("/bot-profiles/:id", async (request, reply) =>
        execute(
          request,
          reply,
          () => {
...
async function testModelConnection(services: AiAdminServices, modelId: string, signal: AbortSignal) {
  ...
  if (!result.ok) {
    throw new AiAdminRouteError(providerErrorStatus(result.error.code), result.error.code, result.error.message);
  }
  return result;
}

async function execute(request: FastifyRequest, reply: FastifyReply, operation: () => unknown | Promise<unknown>, successStatus = 200) {
  try {
    ...
    return reply.code(mapped.status).send(apiError(request, mapped.code, mapped.message));
  }
}
...
function mapRouteError(error: unknown): AiAdminRouteError {
  ...
  if (message.includes("secure secret box")) {
    return new AiAdminRouteError(503, "AI_MASTER_KEY_REQUIRED", "服务端尚未配置 AI_MASTER_KEY，无法安全保存或读取凭证");
  }
  if (message.includes("referenced") || message.includes("fallback") || message.includes("UNIQUE constraint")) {
...
function readId(params: unknown): unknown {
  return typeof params === "object" && params !== null && "id" in params ? params.id : undefined;
}
  ...
  return isAuthorizedAiAdmin(
    {
      ...
      ...(request.headers["x-werewolf-proxy-client-ip"] === undefined ? {} : { proxyClientAddress: request.headers["x-werewolf-proxy-client-ip"] }),
...
function apiError(request: FastifyRequest, code: string, message: string) {
