import { startReadinessSchema, type RoleConfigurationInput, type StartReadiness, type StartReadinessIssue } from "@werewolf/shared";
...
export function evaluateStartReadiness(configuration: RoleConfigurationInput, participantCount: number): StartReadiness {
  ...
  if (configuration.seer + configuration.witch + (configuration.guard ?? 0) + (configuration.hunter ?? 0) + (configuration.idiot ?? 0) < 1) {
