  ...
  return (
    <main className="host-shell">
      ...
      <section className="lobby-workspace">
        ...
        {game ? (
          <section className="host-phase-panel" aria-label="阶段控制">
            <div className="host-phase-summary">
              ...
              {lobby ? <span className={`chat-mode-status mode-${lobby.chatMode}`}>{lobby.chatMode === "open" ? "自由讨论" : "有序发言"}</span> : null}
            </div>
            <div className="phase-controls">
              ...
              <button
                type="button"
                onClick={() => adjustPhaseTime(-15_000)}
                disabled={socket !== "connected" || !["running", "paused"].includes(game.clock.status)}
              >
              ...
              <button
                ...
                disabled={
                  socket !== "connected" ||
                  !["last-words", "day-speech", "day-vote"].includes(lobby?.phase ?? "") ||
                  !["running", "paused"].includes(game.clock.status)
                }
              >
                <ArrowRight size={16} aria-hidden="true" />
                跳过白天阶段
        ...
        {lobby && ["role-reveal", "first-night", "dawn"].includes(lobby.phase) ? (
          <section className="role-confirmation-panel" aria-live="polite">
            ...
            <strong data-testid="role-confirmation-progress">
              {lobby.nightProgress ? "进行中" : `${lobby.roleConfirmation.confirmed} / ${lobby.roleConfirmation.total}`}
            </strong>
        ...
        {lobby?.phase === "exile-result" ? (
          <button className="host-continue-button" type="button" onClick={continueFromExile} disabled={socket !== "connected" || lobby.dayState?.hunterPending}>
            <ArrowRight size={18} aria-hidden="true" />
            {lobby.dayState?.voteResult?.exiledPlayer ? "进入放逐遗言" : "进入下一夜"}
          </button>
        ) : null}
          ...
          <section className="host-day-panel">
            ...
            <section className="host-chat-log" aria-label="公开文字发言">
              <header>
                ...
                <small>
                  {lobby.chatMode === "open" ? "自由讨论" : "有序发言"} · {lobby.publicChat.messages.length} 条
                </small>
              </header>
            ...
            {lobby.phase === "exile-result" && lobby.dayState.voteResult ? (
              <div className="host-ballots">
                {lobby.dayState.voteResult.ballots.map((ballot) => (
                  <span key={ballot.voter.id}>
                    {ballot.voter.number} 号 → {ballot.target ? `${ballot.target.number} 号` : "弃票"}
                  </span>
                ))}
              </div>
            ) : (
              <div className="host-speech-order">
                {lobby.dayState.speechOrder.map((player) => (
                  <span className={player.id === lobby.dayState?.currentSpeaker?.id ? "is-current" : ""} key={player.id}>
        ...
        {lobby?.phase === "game-over" && lobby.gameResult ? (
          <GameOverPanel result={lobby.gameResult} publicMessages={lobby.publicChat.messages} hostActions={{ playAgain, returnToLobby }} />
        ) : null}

        {lobby ? (
          <div className="lobby-grid">
            {lobby.phase === "lobby" ? (
              <section className="join-station" aria-labelledby="join-title">
            ...
            <section className="roster" aria-labelledby="roster-title">
              <div className="roster-header">
                ...
                <strong className="player-count" data-testid="player-count">
              ...
              {lobby.phase === "lobby" ? <HostBotControls players={lobby.players} connected={socket === "connected"} onAddBot={addBot} /> : null}
                    ...
                    <article className="roster-row" key={player.id} data-testid="host-player">
                      ...
                      <div className="row-actions">
                        ...
                        <button
                          ...
                          disabled={lobby.phase !== "lobby" || index === lobby.players.length - 1}
                          title="下移玩家"
                        >
      ...
      <footer className="host-footer">
        <span>
          <ShieldCheck size={16} aria-hidden="true" /> 公共屏仅展示公开信息
        </span>
