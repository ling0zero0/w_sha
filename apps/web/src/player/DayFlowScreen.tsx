...
export function DayFlowScreen({
  ...
  const revealedIdiot = lobby.revealedIdiotId ? lobby.players.find((player) => player.id === lobby.revealedIdiotId) : null;
      ...
      <section className="mobile-state day-flow">
        ...
        <p className="eyebrow">
          {hunterActionActive
            ...
            : lobby.phase === "dawn"
              ? "天亮公布"
              : lobby.phase === "last-words"
                ? "第一夜遗言"
                : lobby.phase === "day-speech"
                  ? "白天发言"
          ...
          {hunterActionActive
            ...
            : lobby.phase === "dawn"
              ...
              : lobby.phase === "exile-result"
                ? day?.voteResult?.exiledPlayer
                  ? `${day.voteResult.exiledPlayer.number} 号被放逐`
                  : "平票，无人放逐"
                : day?.currentSpeaker
                  ? `${day.currentSpeaker.number} 号 · ${day.currentSpeaker.nickname}`
                  : "等待流程推进"}
        </h1>
        ...
        {revealedIdiot ? (
          <div className="idiot-reveal-notice" role="status">
            <strong>
              {revealedIdiot.number} 号 · {revealedIdiot.nickname} 是白痴
            </strong>
            <span>身份已公开，本局不再拥有投票权。</span>
          </div>
        ) : null}
        {hunterAction && (hunterActionActive || hunterAction.submitted) ? (
          <HunterActionPanel action={hunterAction} connected={connected} paused={game?.clock.status === "paused"} onSubmit={onHunterShoot} />
        ) : null}
          ...
          <>
            ...
            <section className="day-chat" aria-label="白天公开聊天">
              <header>
                ...
                <small>{lobby.chatMode === "open" ? "自由讨论" : isCurrentSpeaker ? "轮到你发言" : `等待 ${day?.currentSpeaker?.number ?? ""} 号发言`}</small>
              </header>
            ...
            {isCurrentSpeaker ? (
              <button className="confirm-role-button" type="button" disabled={!connected} onClick={onFinishSpeaking}>
                结束我的发言
              </button>
            ) : null}
          </>
            ...
            <div className="day-vote-panel">
              ...
              <div className="day-vote-options">
                {lobby.dayVote.candidates.map((candidate) => (
                  <button
                    className={lobby.dayVote?.target === candidate.id ? "is-selected" : ""}
                    type="button"
                    key={candidate.id}
                    disabled={lobby.dayVote?.confirmed}
                    onClick={() => onSelectVote(candidate.id)}
                  >
                ...
                <button
                  className={lobby.dayVote.target === "abstain" ? "is-selected" : ""}
                  type="button"
                  disabled={lobby.dayVote.confirmed}
                  onClick={() => onSelectVote("abstain")}
                >
                  弃票
                </button>
              </div>
              <button
                ...
                type="button"
                disabled={!connected || lobby.dayVote.target === null}
                onClick={() => onConfirmVote(!lobby.dayVote?.confirmed)}
              >
                {lobby.dayVote.confirmed ? "取消确认并修改" : "确认投票"}
              </button>
              <small className="confirmation-count">
                已确认 {day?.voteProgress?.confirmed ?? 0} / {day?.voteProgress?.total ?? 0}
              </small>
            </div>
          ) : (
            <div className="day-ineligible-notice">
              {lobby.revealedIdiotId === lobby.selfId ? "你的白痴身份已揭示，本局失去投票权" : "你已死亡，等待存活玩家投票"}
            </div>
          )
        ) : null}
        {lobby.phase === "exile-result" && day?.voteResult ? (
          <div className="ballot-result">
            {day.voteResult.ballots.map((ballot) => (
              <div key={ballot.voter.id}>
                <strong>
                  {ballot.voter.number} 号 {ballot.voter.nickname}
                </strong>
                <span>投给</span>
                <b>{ballot.target ? `${ballot.target.number} 号 ${ballot.target.nickname}` : "弃票"}</b>
              </div>
            ))}
          </div>
        ) : null}
        <PublicPlayerRoster players={lobby.players} selfId={lobby.selfId} phase={lobby.phase} />
        <p className="day-self-state">
          {self ? `${self.number} 号 · ${self.nickname}` : "玩家"} · {alive ? "存活" : "死亡"}
        </p>
      </section>
    </main>
  );
