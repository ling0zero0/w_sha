...
export function GameOverPanel({
  ...
  const replayMessages = publicMessages.filter((message) => message.channel === "day-public" || message.channel === "system");
    ...
    <section className="game-over-panel">
      ...
      <div className="revealed-roles">
        {result.revealedPlayers.map((player) => (
          <article key={player.id}>
            ...
            <span>
              {player.number} 号 · {player.nickname}
            </span>
      ...
      {hostActions ? (
        <div className="game-over-actions">
          <button type="button" onClick={hostActions.playAgain}>
            再来一局
          </button>
          <button type="button" onClick={hostActions.returnToLobby}>
            返回大厅调整
          </button>
        </div>
      ) : null}
