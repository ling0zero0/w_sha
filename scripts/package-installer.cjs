...
result = spawnSync(compiler, [`/DMyAppVersion=${packageJson.version}`, path.join(projectRoot, "installer", "W_SHA.iss")], {
  cwd: projectRoot,
  stdio: "inherit",
  windowsHide: true
});
...
result = spawnSync(process.execPath, [path.join(__dirname, "verify-release-artifacts.cjs")], { cwd: projectRoot, stdio: "inherit", windowsHide: true });
