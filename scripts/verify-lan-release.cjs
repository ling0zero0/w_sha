...
const { delay, projectRoot, resolveReleasePort, selectLanAddress, stopChild } = require("./lib/verify-utils.cjs");
...
async function main() {
  ...
  try {
    const port = await resolveReleasePort(DEFAULT_RELEASE_PORT);
