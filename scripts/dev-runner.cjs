...
function isWebReady() {
  return new Promise((resolve) => {
    const request = http.get(webUrl, (response) => {
      ...
      response.on("end", () => {
        resolve(response.statusCode >= 200 && response.statusCode < 400 && body.includes('data-app="werewolf-lan"'));
      });
    });
...
async function main() {
  ...
  if (occupiedPorts.length > 0) {
    throw new Error(`Port ${occupiedPorts.join(" and ")} is already in use. Stop the other project before starting W_SHA.`);
  }
